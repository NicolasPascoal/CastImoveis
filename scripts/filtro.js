...
async function carregarImoveis(filtros = {}) {
  try {
    ...
    if (filtros.tipo) {
      lista = lista.filter(i => i.tipo.toLowerCase() === filtros.tipo.toLowerCase());
    }

    if (filtros.cidade) {
      lista = lista.filter(i => i.cidade.toLowerCase() === filtros.cidade.toLowerCase());
    }

    if (filtros.estado) {
      lista = lista.filter(i => i.estado.toLowerCase() === filtros.estado.toLowerCase());
    }

    if (filtros.finalidade) {
      lista = lista.filter(i => i.finalidade.toLowerCase() === filtros.finalidade.toLowerCase());
    }

    if (filtros.precoMax) {
      lista = lista.filter(i => i.preco <= filtros.precoMax);
    }

    if (filtros.precoMin) {
      lista = lista.filter(i => i.preco >= filtros.precoMin);
    }
